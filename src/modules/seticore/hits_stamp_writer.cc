...
template<typename IT>
Result HitsStampWriter<IT>::process(const cudaStream_t& stream) {
    ...
    for (const DedopplerHitGroup& group : groups) {
        ...
        U64 lowIndex = inputShape.numberOfFrequencyChannels();
        U64 highIndex = 0;
        for (const DedopplerHit& hit : group.hits) {
            if ((U64) hit.lowIndex() < lowIndex) {
                lowIndex = hit.lowIndex();
            }
            if ((U64) hit.highIndex() > highIndex) {
        ...
        const U64 last_channel = highIndex > inputShape.numberOfFrequencyChannels() ? inputShape.numberOfFrequencyChannels() : (U64) highIndex;
        ...
        const ArrayShape regionOfInterestDims = ArrayShape({
            inputShape.numberOfAspects(),
            (U64) (last_channel - first_channel),
        ...
        if (this->stampsWritten == 0) {
            auto filepath = bl::fmt::format("{}.seticore.{:04}.stamps", this->config.filepathPrefix, this->fileId);
