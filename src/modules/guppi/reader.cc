...
Reader<OT>::Reader(const Config& config,
    ...
    const auto totalShape = getTotalOutputBufferShape();
    auto stepShape = getStepOutputBufferShape();
    BL_INFO("Type: {} -> {}", "N/A", TypeInfo<OT>::name);
    BL_INFO("Step Shape: {} -> {}", "N/A", stepShape);
    ...
    BL_INFO("Total Shape: {} -> {} (across {} {})", "N/A", totalShape, this->gr_iterate.n_file, this->gr_iterate.n_file > 1 ? "files" : "file");
    BL_INFO("Steps in Dimensions: {}", getNumberOfStepsInDimensions());
    ...
    output.stepFrequencyChannelOffset = Tensor<Device::CPU, U64>({1});
...
template<typename OT>
F64 Reader<OT>::getChannelTimespan() const {
    return getBlockMeta(&gr_iterate)->chan_timespan;
}

template<typename OT>
F64 Reader<OT>::getObservationBandwidth() const {
    return getChannelBandwidth() * getBlockMeta(&gr_iterate)->fenchan;
}

template<typename OT>
F64 Reader<OT>::getBandwidth() const {
...
template<typename OT>
F64 Reader<OT>::getCenterFrequency() const {
    return (double)getBlockMeta(&gr_iterate)->obs_freq_mhz * 1e6;
}

template<typename OT>
F64 Reader<OT>::getObservationCenterFrequency() const {
...
template<typename OT>
F64 Reader<OT>::getObservationBottomFrequency() const {
    return getCenterFrequency() + (
        -(double)getChannelStartIndex()
        - ((double)this->getDatashape()->n_aspectchan) / 2.0
    ) * getChannelBandwidth();
...
template<typename OT>
F64 Reader<OT>::getBottomFrequency() const {
    return getCenterFrequency() + (
        - ((double)this->getDatashape()->n_aspectchan) / 2.0
    ) * getChannelBandwidth();
}
...
template<typename OT>
F64 Reader<OT>::getObservationTopFrequency() const {
    return getCenterFrequency() + (
        -((double)getChannelStartIndex())
        - (((double)this->getDatashape()->n_aspectchan) / 2.0)
...
template<typename OT>
F64 Reader<OT>::getTopFrequency() const {
    return getCenterFrequency() + (
        + (((double)this->getDatashape()->n_aspectchan) / 2.0)
    ) * getChannelBandwidth();
}
...
Result Reader<OT>::process(const U64& currentStepCount, const Stream& stream) {
    ...
    auto stepOutputBufferShape = this->getStepOutputBufferShape();

    // Run library internal read method.
    const I64 bytes_read = 
        guppiraw_iterate_peek(&this->gr_iterate,
                              stepOutputBufferShape.numberOfTimeSamples(),
                              stepOutputBufferShape.numberOfFrequencyChannels(),
                              stepOutputBufferShape.numberOfAspects(),
    ...
    if (config.numberOfTimeSampleStepsBeforeFrequencyChannelStep > 1) {
        ...
        // special case, iterate all time before incrementing channel
        this->current_time_sample_step += 1;
        ...
        if (
            ...
            && guppiraw_iterate_ntime_remaining(&this->gr_iterate) <
                this->config.requiredMultipleOfTimeSamplesSteps * stepOutputBufferShape.numberOfTimeSamples()
        ) {
